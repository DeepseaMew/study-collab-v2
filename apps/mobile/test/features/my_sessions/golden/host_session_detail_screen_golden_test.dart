...
void main() {
  group('HostSessionDetailScreen golden', () {
    testWidgets('scale 1.0 — th locale', (tester) async {
      await mockNetworkImagesFor(() async {
        await tester.pumpWidget(_buildScreen(1.0));
        await tester.pumpAndSettle(const Duration(seconds: 3));
        await expectLater(
          find.byType(HostSessionDetailScreen),
          matchesGoldenFile(
            'goldens/host_session_detail_screen_scale_1.0_th.png',
          ),
        );
      });
    });
    ...
    testWidgets('scale 1.5 — th locale', (tester) async {
      await mockNetworkImagesFor(() async {
        ...
        await expectLater(
          find.byType(HostSessionDetailScreen),
          matchesGoldenFile(
            'goldens/host_session_detail_screen_scale_1.5_th.png',
          ),
        );
      });
    });
