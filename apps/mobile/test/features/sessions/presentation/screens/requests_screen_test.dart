...
class _MockJoinRequestRepository extends Mock implements JoinRequestRepository {}
...
Widget _buildScreen({
  AsyncValue<List<JoinRequestEntity>> requestsState =
      const AsyncValue.data([]),
  bool authenticated = true,
}) {
  final mockRepo = _MockJoinRequestRepository();
  when(() => mockRepo.approveRequest(any(), any(), any()))
      .thenAnswer((_) async {});
  when(() => mockRepo.declineRequest(any(), any(), any()))
      .thenAnswer((_) async {});
  when(() => mockRepo.watchRequests(any()))
  ...
  return ProviderScope(
    overrides: [
      firebaseAuthStateProvider.overrideWith(
        (_) => Stream.value(
          authenticated ? _FakeFirebaseUser(_callerUid) : null,
        ),
      ),
...
void main() {
  testWidgets(
    'RequestsScreen — renders "Join Requests" title',
    (tester) async {
  ...
  testWidgets(
    'RequestsScreen — unauthenticated shows sign-in prompt',
    (tester) async {
      await mockNetworkImagesFor(() async {
  ...
  testWidgets(
    'RequestsScreen — empty list shows "No pending requests."',
    (tester) async {
      await mockNetworkImagesFor(() async {
        await tester.pumpWidget(_buildScreen());
        await tester.pumpAndSettle(const Duration(seconds: 3));
  ...
  testWidgets(
    'RequestsScreen — error state shows "Could not load requests."',
    (tester) async {
      await mockNetworkImagesFor(() async {
        await tester.pumpWidget(_buildScreen(
  ...
  testWidgets(
    ...
    (tester) async {
      await mockNetworkImagesFor(() async {
        await tester.pumpWidget(_buildScreen(
          requestsState: AsyncValue.data([_stubRequest()]),
        ));
        await tester.pumpAndSettle(const Duration(seconds: 3));
