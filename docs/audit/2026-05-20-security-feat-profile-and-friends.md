...
- **apps/mobile/lib/firebase_options.dart lines 53 and 62 - Firebase API keys committed in source** -> Two apiKey values are committed in plain text (Web: AIzaSyDkiFO_GRKRE9O8TrULUikYWesF9f9yeMI, Android: AIzaSyDQ4YJThrTC15zI3g6xvVOZIPyyy9xvo88). CLAUDE.md states no secrets or API keys in source; Firebase config injected via CI environment only. Required fix: remove firebase_options.dart from version control; add to .gitignore; inject via CI; rotate the exposed keys.
