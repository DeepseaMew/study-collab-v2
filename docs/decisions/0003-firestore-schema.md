...
### Amendment A — Add `faculty` to `users/{uid}`; add `hostFaculty` to `sessions/{sessionId}`; add Index 8

**Date:** 2026-05-16

Fields added to `users/{uid}`: `faculty` (String, `.name` of `KmuttFaculty` enum). Field added to `sessions/{sessionId}`: `hostFaculty` (String, copied from `users/{hostUid}.faculty` at creation, immutable). Schema tables and rules sketch updated in place above.

Index 8 added: `sessions` — `hostFaculty` asc + `status` asc + `scheduledAt` asc — for faculty-filtered session search (see index table above).

Rules impact: `faculty` added to `users/{uid}` create `hasAll` and update `hasOnly`; `hostFaculty` added to `sessions` create `hasAll` and immutability check in update rule.
...
`department` field dropped entirely; `KmuttDepartment` enum removed from codebase. `bio` (String, nullable, max 300 characters) added in its place. Schema table and rules sketch updated in place above.

Rules impact: `department` removed from `users/{uid}` create `hasAll` and update `hasOnly`; `bio` added to update `hasOnly` (not required at creation).
...
`fullName` (String, required, max 150 characters, **PII — never log**) added to `users/{uid}`. Schema table and rules sketch updated in place above.

Rules impact: `fullName` added to create `hasAll` and update `hasOnly`.

---

### Amendment D — `fullName` written at signup; `displayName` placeholder written at signup

**Date:** 2026-05-16

`fullName` is written with its real value by `signUpWithEmail` (the initial `set` call in `AuthRepositoryImpl`). The `completeProfile` update call must **not** write `fullName`.

`displayName`, `faculty`, and `bio` are written as `''` placeholders in `signUpWithEmail` and overwritten with real values by `completeProfile`.

| Write call | `fullName` | `displayName` |
|---|---|---|
| `signUpWithEmail` (create) | Real trimmed value from signup form | `''` placeholder |
| `completeProfile` (update) | **Not written** | Real trimmed value from profile-setup form |
